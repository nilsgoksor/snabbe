...
const PlayerProfilesPage = () => {
  ...
  return (
    <>
      ...
      {fetchedRounds.length === 0 || fetchedPlayers.length === 0? ('') : (
        <>
          <SelectTest value={selectedPlayer} onChange={(event) => setPlayer(event.target.value)}>
            {options}
          </SelectTest>
  ...
  function createDropdownOptions(playerNames) {
    ...
    for (const player of playerNames) {
      options.push(
          <option key={player} value={player} label={player.toLowerCase()}> </option>
        )
    }
    return options
  }
  ...
  function getInfo (player, isWeakness) {
    switch (player) {
      case 'ADAM':
        return isWeakness ? [<li key="1">Sjukt inconsistent</li>] : [<li key="1">Var 20:e skott är otagbart</li>, <li key="2">Långa armar</li>]
      case 'ANTON':
        return isWeakness ? [<li key="1">Kan inte skjuta med bra bollar</li>] : [<li key="1">Är alltid med</li>, <li key="2">Konsistent</li>]
      case 'CRIPPE':
        return isWeakness ? [<li key="1">Har ofta tvättid</li>] : [<li key="1">Skjuter som en hest</li>]
      case 'IDA':
        return isWeakness ? [<li key="1">Helsingborg</li>] : [<li key="1">Makt över Adam</li>, <li key="2">Blir inte trött</li>]
      case 'JOEL':
        return isWeakness ? [<li key="1">Svaga knän</li>] : [<li key="1">Kan skjuta med fel fot</li>]
      case 'JOHAN':
        return isWeakness ? [<li key="1">Malmö</li>] : [<li key="1">Kan spela målvakt</li>]
      case 'JOHANNES':
        return isWeakness ? [<li key="1">Nils passningar</li>, <li key="2">Noll kondition</li>] : [<li key="1">Målvaktshandskar</li>, <li key="2">Slänger sig i mål</li>]
      case 'NILS':
        return isWeakness ? [<li key="1">Vänsterfotad</li> , <li key="2">Lätt-tiltad</li>, <li key="3">Måste äta middag</li>] : [<li key="1">Skjuter bäst i gänget</li>, <li key="2">Bor på Lerbäck</li>]
      case 'TOBBE':
        return isWeakness ? [<li key="1">Gammal - går sönder efter tre rundor</li>, <li key="2">Kan inte skjuta på stillaliggande boll</li>] : [<li key="1">Kan lobba över Anton</li>]
      default:
  ...
  function addImagesToPlayerData() {
    ...
    const adamImages = importAll(require.context('../media/playerImages/adam', false, /\.(png|jpe?g|svg)$/));
    const antonImages = importAll(require.context('../media/playerImages/anton', false, /\.(png|jpe?g|svg)$/));
    const crippeImages = importAll(require.context('../media/playerImages/crippe', false, /\.(png|jpe?g|svg)$/));
    const idaImages = importAll(require.context('../media/playerImages/ida', false, /\.(png|jpe?g|svg)$/));
    const joelImages = importAll(require.context('../media/playerImages/joel', false, /\.(png|jpe?g|svg)$/));
    const johanImages = importAll(require.context('../media/playerImages/johan', false, /\.(png|jpe?g|svg)$/));
    const johannesImages = importAll(require.context('../media/playerImages/johannes', false, /\.(png|jpe?g|svg)$/));
    const nilsImages = importAll(require.context('../media/playerImages/nils', false, /\.(png|jpe?g|svg)$/));
    const tobbeImages = importAll(require.context('../media/playerImages/tobbe', false, /\.(png|jpe?g|svg)$/));
    ...
    if(fetchedRounds.length !== 0 && fetchedPlayers.length !== 0) {
      ...
      data['IDA'].images = idaImages
      data['JOEL'].images = joelImages
  ...
  function getCompiledData() {
    ...
    for(const round of fetchedRounds) {
      for(const player of round) {
        if(compiledData[player.name]) {
          compiledData[player.name].points = compiledData[player.name].points + player.points
    ...
    for(var name in compiledData) {
      ...
      compiledData[name].pointsPerRound = (compiledData[name].points - compiledData[name].initialPoints) / compiledData[name].rounds 
    }
